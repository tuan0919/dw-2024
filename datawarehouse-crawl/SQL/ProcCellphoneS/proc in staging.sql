...
begin
	...
	*/
	...
	into 
		file_paths, fields_terminated, optionally_enclosed, lines_terminated, 
		ignore_row, stg_fields, log_id, table_staging
	from 
	...
	join 
		...
		on fl.config_id = cf.config_id      -- Kết nối bảng `file_logs` với `configs` dựa trên `config_id` (khóa ngoại).
	join 
		dbcontrol.process_properties pp     -- Bảng chứa thuộc tính xử lý, như danh sách cột tương ứng với tệp CSV.
		on pp.property_id = cf.property_id  -- Kết nối bảng `configs` với `process_properties` dựa trên `property_id`.
	where 
...
CREATE PROCEDURE update_file_status(
    IN log_id INT,           -- Tham số đầu vào: ID của file log cần cập nhật.
    IN new_status VARCHAR(20), -- Tham số đầu vào: Trạng thái mới sẽ được cập nhật cho file log
    IN start_time DATETIME,  -- Tham số đầu vào: Thời gian bắt đầu xử lý file log.
    IN end_time DATETIME     -- Tham số đầu vào: Thời gian kết thúc xử lý file log.
)
BEGIN
    -- Cập nhật trạng thái và thời gian vào bảng file_logs
    UPDATE dbcontrol.file_logs
    SET 
        status = new_status,        -- Gán giá trị `new_status` vào cột `status`.
        start_time = start_time,    -- Gán giá trị `start_time` vào cột `start_time`.
        end_time = end_time         -- Gán giá trị `end_time` vào cột `end_time`.
    WHERE 
        file_log_id = log_id; -- Điều kiện: Chỉ cập nhật bản ghi có `file_log_id` bằng với `log_id` truyền vào.
END //
